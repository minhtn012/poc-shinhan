        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead class="w-40">Versions</TableHead>
          ...
          <TableBody>
            <TableRow
              ...
              @click="router.push('/templates/' + tmpl.id)"
            >
              <TableCell class="font-medium">{{ tmpl.name }}</TableCell>
              <TableCell class="text-sm">{{ tmpl.versionCount ?? 0 }} versions</TableCell>
              <TableCell class="text-sm text-muted-foreground">{{ formatDate(tmpl.createdAt) }}</TableCell>
              <TableCell @click.stop>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" class="w-40">
                    <DropdownMenuItem @click="openEditDialog(tmpl)">
                      <Pencil class="size-4 mr-2" /> Edit
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem class="text-destructive focus:text-destructive" @click="handleDelete(tmpl.id)">
...
function openEditDialog(tmpl: Template): void {
...
async function saveEdit(): Promise<void> {
  try {
    await templateService.update(editForm.value.id, {
